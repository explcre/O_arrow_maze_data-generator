"""
Arrow Maze Navigation Task Generator - Improved version with reduced boundary/loop probability.

Features:
- Only 10% of cells have arrows pointing to boundaries or creating immediate loops
...
class TaskGenerator(BaseGenerator):
    ...
    def _generate_task_data(self) -> dict:
        ...
        grid = self._generate_smart_grid(grid_size)
    ...
    def _generate_smart_grid(self, grid_size: int) -> List[List[str]]:
        """Generate grid with only 10% of cells having boundary-pointing or loop-creating arrows."""
        grid = [[None for _ in range(grid_size)] for _ in range(grid_size)]
        total_cells = grid_size * grid_size
        problematic_quota = int(total_cells * 0.1)
        problematic_count = 0
        ...
        for row, col in positions:
            valid_directions = self._get_valid_directions(grid, row, col, grid_size)
            problematic_directions = self._get_problematic_directions(
                grid, row, col, grid_size
            )

            # If we haven't filled the problematic quota and randomly decide to add one (weighted)
            if (
                problematic_count < problematic_quota
                and problematic_directions
                and random.random() < 0.15
            ):
                grid[row][col] = random.choice(problematic_directions)
                problematic_count += 1
            elif valid_directions:
                # Choose from valid (non-problematic) directions
                grid[row][col] = random.choice(valid_directions)
            else:
                # Fallback: choose any direction if no valid ones exist
                grid[row][col] = random.choice(list(self.DIRECTIONS.keys()))

        return grid

    def _get_valid_directions(
        self, grid: List[List[str]], row: int, col: int, grid_size: int
    ) -> List[str]:
        """Get directions that don't point to boundary or create immediate loops."""
        valid = []

        for direction, (dx, dy) in self.DIRECTIONS.items():
            next_col, next_row = col + dx, row + dy

            # Check if points to boundary
            if not (0 <= next_row < grid_size and 0 <= next_col < grid_size):
                continue

            # Check if creates immediate loop (neighbor points back)
            neighbor_direction = grid[next_row][next_col]
            if neighbor_direction:
                ndx, ndy = self.DIRECTIONS[neighbor_direction]
                back_col, back_row = next_col + ndx, next_row + ndy
                if (back_row, back_col) == (row, col):
                    continue
    ...
    def _get_problematic_directions(
        self, grid: List[List[str]], row: int, col: int, grid_size: int
    ) -> List[str]:
        """Get directions that point to boundary or create immediate loops."""
        problematic = []
        ...
        for direction, (dx, dy) in self.DIRECTIONS.items():
            ...
            # Points to boundary
            if not (0 <= next_row < grid_size and 0 <= next_col < grid_size):
                problematic.append(direction)
                continue

            # Creates immediate loop
            neighbor_direction = grid[next_row][next_col]
            if neighbor_direction:
                ndx, ndy = self.DIRECTIONS[neighbor_direction]
                back_col, back_row = next_col + ndx, next_row + ndy
    ...
    def _draw_arrow(self, draw, cx, cy, direction, size, color):
        ...
        if direction == "up":
            draw.rectangle(
                [cx - shaft_width, cy - half + head_size, cx + shaft_width, cy + half],
                fill=color,
            )
            draw.polygon(
                [
            ...
            draw.rectangle(
                [cx - shaft_width, cy - half, cx + shaft_width, cy + half - head_size],
                fill=color,
            )
            draw.polygon(
                [
            ...
            draw.rectangle(
                [cx - half + head_size, cy - shaft_width, cx + half, cy + shaft_width],
                fill=color,
            )
            draw.polygon(
                [
            ...
            draw.rectangle(
                [cx - half, cy - shaft_width, cx + half - head_size, cy + shaft_width],
                fill=color,
            )
            draw.polygon(
                [
    ...
    def _render_grid(
        ...
        if highlight_end:
            ...
            draw.rectangle(
                ...
                outline=self.config.destination_color,
                width=4,
            )
        ...
        for i in range(grid_size + 1):
            ...
            draw.line(
                [(x, start_y), (x, start_y + grid_size * cell_size)],
                fill=(*self.config.grid_color, 255),
                width=2,
            )
            y = start_y + i * cell_size
            draw.line(
                [(start_x, y), (start_x + grid_size * cell_size, y)],
                fill=(*self.config.grid_color, 255),
                width=2,
            )
    ...
    def _render_initial_state(self, task_data: dict) -> Image.Image:
        return self._render_grid(
            task_data, agent_pos=(task_data["start_row"], task_data["start_col"])
        )
    ...
    def _render_final_state(self, task_data: dict) -> Image.Image:
        return self._render_grid(
            ...
            visited_cells=task_data["path"],
            highlight_end=True,
        )

    def _generate_video(
        self, first_image, final_image, task_id: str, task_data: dict
    ) -> str:
        ...
        for i, (row, col) in enumerate(path):
            ...
            frame = self._render_grid(
                task_data,
                agent_pos=(row, col),
                visited_cells=visited_so_far,
                highlight_end=is_last,
            )
